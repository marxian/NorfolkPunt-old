...
class Event(models.Model):
    ...
    def get_filterable_classes(self):
        klasses = ['event', slugify(self.type)]
        if self.results.count():
            klasses.append('results')
            
        for flag in self.flags.all():
            klasses.append(slugify(flag.name))
            klasses.append(slugify(flag.name + str(self.start.year)))
